...
2. Update the Homebrew formula: `Formula/lingbox.rb` in [wgzhao/homebrew-tap](https://github.com/wgzhao/homebrew-tap) (bump `url` tag + `sha256` of the new tarball), push to the tap repo. The formula builds from source on all platforms; the codebase is pure Go (no cgo), so there are no external build dependencies.
